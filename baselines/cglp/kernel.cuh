                                        ...
                                        SIZE_TYPE *row)
{
    ...
    for (int i = block_id; i < end; i += steps)
    {
        ...
        __shared__ typename Decoder2<4, 100, 128>::SMem_b smem;
        Decoder2<4, 100, 128> decoder(&smem, graph, offsets, row, v, i);
        decoder.decompress();
    }
}
                                      ...
                                      SIZE_TYPE *row)
{
    ...
    for (int i = warpid_in_all + begin; i < end; i += steps)
    {
        __shared__ typename Decoder2<4, 100, 128>::SMem_b smem;
        SIZE_TYPE v = vertices[i];
        Decoder2<4, 100, 128> decoder(&smem, graph, offset, row, v, i);
        decoder.decompress();
    }
}
...
template <typename V> __global__ void initialize_labels(V *labels, V n)
{
...
template <typename V, typename E> __global__ void gather_labels(const V *neighbors, const int *labels, E *dst, E m)
{
    int gid = get_gid();
...
template <typename V, typename E> __global__ void find_boundaries(V *adj_labels, E m, int *flags)
{
    int gid = get_gid();
    if (gid < m - 1)
    {
...
template <typename V, typename E> __global__ void set_boundary_case(E *offsets, V n, int *flags)
{
    int gid = get_gid();
    if (gid < n)
    {
...
__global__ void scatter_indexes(int *flags, E *offsets, V n, int m, int *boundaries, int *boundary_offsets)
{
    int gid = get_gid();
    if (gid < m)
    {
...
__global__ void compute_label_weights(int *boundaries, int *limitp, int *label_weights)
{
    int limit = limitp[0];
...
template <typename V>
__global__ void update_labels(int *adj_labels, int *boundaries, int *label_index, V n, int *labels, int *counter)
{
    int gid = get_gid();
...
__global__ void compute_max_labels(int *seg_offsets, int *segments, int *adj_labels, int *labels, int n, int *counter)
{
...
#pragma unroll
            for (int i = 1; i < 4; ++i)
            {
                if (_update(my_weight, s_weights[32 * i + threadIdx.x], my_index, s_indexes[32 * i + threadIdx.x]))
                {
...
#pragma unroll
                for (int stride = 16; stride > 0; stride >>= 1)
                {
                    ...
                    {
                        if (_update(vs_weights[threadIdx.x], vs_weights[threadIdx.x + stride], vs_indexes[threadIdx.x],
                                    vs_indexes[threadIdx.x + stride]))
                        {
