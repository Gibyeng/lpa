...
template <typename V, typename E> class NovaPropagator : public LabelPropagator<V, E>
{
  public:
    using typename LabelPropagator<V, E>::CGRGraphT;
    NovaPropagator(std::shared_ptr<CGRGraphT> _G) : LabelPropagator<V, E>(_G->n), G(_G)
    {
    }
...
template <typename V, typename E> void NovaPropagator<V, E>::errorCheck(std::string message)
{
...
template <typename V, typename E> std::pair<double, double> NovaPropagator<V, E>::run(int niter)
{
    Timer t1, t2;
    t2.start();
    preprocess();
...
template <typename V, typename E> int NovaPropagator<V, E>::iterate(int i)
{
    ...
    if (num_big > 0)
    {
        ...
        Timer t_big;
        t_big.start();
        propagate_large_vertex<<<b_block_num, 128>>>(big_index, d_cgr_neighbors, d_vertices, d_cgr_offsets,
    ...
    if (num_medium > 0)
    {
        ...
        propagate_mid_vertex<<<m_block_num, 128>>>(big_index, medium_index, d_cgr_neighbors, d_vertices, d_cgr_offsets,
                                                   d_csr_offsets, d_labels,G->n);
        cudaDeviceSynchronize();
    ...
    if (num_small > 0)
    {
        ...
        l_small_update_syn_labelload<32, 1, 64><<<nb_small, nt_small>>>(
            medium_index, num_small, d_small_neighbors, d_small_offsets, d_labels, d_labels, d_vertices, d_counter);
        cudaDeviceSynchronize();
        errorCheck("after small nodes LP!");
        t_small.stop();
        cout << "update small nodes label elapsed: " << t_small.elapsed_time() << endl;
        cout << "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" << endl;
    }
    if (num_tiny > 0)
    {
        ...
        Timer t_tiny;
        t_tiny.start();
        l_tiny_update_syn2_labelload<<<nb_tiny2, nt_tiny>>>(d_small_neighbors, d_small_offsets, d_labels, d_labels,
                                                            d_counter, d_warp_v, d_warp_begin, warpnumber);
        cudaDeviceSynchronize();
        errorCheck("after tinys nodes LP!");
        t_tiny.stop();
        cout << "update tiny nodes label elapsed: " << t_tiny.elapsed_time() << endl;
        cout << "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" << endl;
    }
    t_all.stop();
...
template <typename V, typename E> void NovaPropagator<V, E>::preprocess()
{
...
template <typename V, typename E> void NovaPropagator<V, E>::tiny_node_assignment()
{
    num_big = 0;
    num_medium = 0;
    num_small = 0;
    ...
    // computer nodes size
    big_index = binary_search(SIZE_TYPE(0), G->n - 1, G->csr_offsets, G->vertices, big);
    printf("\nbig_index %d \n", big_index);
    medium_index = medium == 0 ? G->n - 1 : binary_search(SIZE_TYPE(0), G->n - 1, G->csr_offsets, G->vertices, medium);
    printf("medium_index %d \n", medium_index);

    if (G->small_node_thresh > 16)
    {
        // in this case , both small nodes and tiny nodes exist
        small_index = binary_search(SIZE_TYPE(0), G->n - 1, G->csr_offsets, G->vertices, small);
    ...
    for (int j = 0; j < num_tiny; j++)
    {
        ...
        {
            warpnumber++;
            int align_minus_1 = 32 - (G->csr_offsets[node] - G->csr_offsets[start_node]);
...
template <typename V, typename E> void NovaPropagator<V, E>::init_gmem()
{
    {
...
template <typename V, typename E> void NovaPropagator<V, E>::transfer_data()
{
    ...
    cudaMemcpy(d_small_neighbors, &G->csr_small_neighbors[0], sizeof(V) * m2, cudaMemcpyHostToDevice);

    cudaMemcpy(d_small_offsets, &G->csr_small_offsets[0], sizeof(V) * n2, cudaMemcpyHostToDevice);

    cudaMemcpy(d_vertices, &G->vertices[0], sizeof(V) * n, cudaMemcpyHostToDevice);

    t.stop();
    cout << "n= " << n << " m1= " << m1 << " m2= " << m2 << endl;
    cout << "transfer data: [ 3n+m1+m2= " << 3 * n + m1 + m2 << " ] elapse: " << t.elapsed_time() << endl;
    cout << "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" << endl;
}

template <typename V, typename E> void NovaPropagator<V, E>::postprocess()
{
    free_gmem();
}

template <typename V, typename E> void NovaPropagator<V, E>::free_gmem()
{
    cudaFree(d_labels);
...
template <typename V, typename E> int NovaPropagator<V, E>::get_count()
{
    int counter;
