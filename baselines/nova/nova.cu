...
const string dblp = "/home/yuchen/tmq/dataset/dblp/com-dblp.ungraph.txt"; // true
const string pokec = "/home/yuchen/tmq/test_graph/pokec.edgelist";
const string roadnet = "/home/yuchen/tmq/dataset/roadnet/roadNet-CA.txt";          // false
const string youtube = "/home/yuchen/tmq/dataset/youtube/com-youtube.ungraph.txt"; // true
const string journal = "/home/yuchen/tmq/dataset/ljournal/soc-LiveJournal1.txt";   // false
...
int main(int argc, char *argv[])
{
    ...
    // Load a graph
    std::shared_ptr<GraphT> graph(new CGRGraph<Vertex, Edge>(f.clm, f.row, f.graph, f.offset, f.small_graph,
                                                             f.small_offset, f.vertices, small_node_thresh));
    std::unique_ptr<Propagator> propagator;

    propagator = make_unique_ptr<NovaPropagator<Vertex, Edge>>(graph);
    ...
    cout << "f1: " << f1/1000 << "(s)" << ",f2: " << f2 << endl;
    //output file
    if(argc != 1){
	writefile.open(outputfile,std::ios::app);
        writefile << "nova file: " << inputfile <<std::endl<< " running time: " << f1/1000 << "thre: " << small_node_thresh <<std::endl;
	writefile <<"~~~~~"<< std::endl;        
	writefile.close();
    }
    return 0;
